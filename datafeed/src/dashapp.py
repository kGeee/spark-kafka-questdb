...
url="http://provider.akashprovid.com:31450"
...
def main():
    ...
    if "sleep_time" not in st.session_state:
        st.session_state.sleep_time = 5
    if "latestTs" not in st.session_state: 
        st.session_state['latestTs'] = str(get_latest_ts())
    ...
    with col3: 
        ...
        if tf: liq_lookback(tf)
    with col4: liq_count_and_amount()

    one_hour_sampled()

    ticker = st.text_input("ticker", value="OPUSDT")
